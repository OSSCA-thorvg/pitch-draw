...
import { SING_HIGH_HZ, SING_LOW_HZ, type Melody } from '../engine/melody.js';
...
export async function audioFileToMelody(file: File, label: string): Promise<Melody> {
...
function samplesFromAudio(audio: AudioBuffer): RecordedPitchSample[] {
  ...
  for (let start = 0; start + FRAME_SIZE <= duration * audio.sampleRate; start += step) {
    ...
    if (pitch) out.push({ t: start / audio.sampleRate, hz: intoVoiceRange(pitch.hz) });
  }

  return out;
}

function intoVoiceRange(hz: number): number {
  let out = hz;
  while (out > SING_HIGH_HZ) out /= 2;
  while (out < SING_LOW_HZ) out *= 2;
  return out;
}

function fillFrame(audio: AudioBuffer, start: number, frame: Float32Array): void {
  frame.fill(0);
  let peak = 0;
  ...
  for (let i = 0; i < frame.length; i++) frame[i] = frame[i]! * gain;
}
