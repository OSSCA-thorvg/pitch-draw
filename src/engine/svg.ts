...
/** 가장 긴 도형 위에서 등간격으로 뽑은 점들. */
export interface PathSamples {
  /** **SVG 좌표계** 위의 점이다. 스테이지에 맞추는 일은 score.ts 가 한다. */
 ...
 */
export function samplePath(svgText: string): PathSamples {
  ...
  try {
    const geoms = [...host.querySelectorAll<SVGGeometryElement>(GEOMETRY_SELECTOR)]
      .filter((el) => typeof el.getTotalLength === 'function');
    if (!geoms.length) throw new Error('그릴 수 있는 도형이 없습니다.');
    ...
    if (!longest) throw new Error('도형의 길이가 0이라 악보로 쓸 수 없습니다.');
 ...
 */
function parseSafely(svgText: string, retried = false): SVGElement {
  ...
  if (svg.namespaceURI !== SVG_NS) {
    if (retried) throw new Error('SVG 이름공간을 알아볼 수 없습니다.');
  ...
  for (const el of [svg, ...svg.querySelectorAll('*')]) {
    for (const attr of [...el.attributes]) {
      ...
      if (external) el.removeAttribute(attr.name);
    }
  }
