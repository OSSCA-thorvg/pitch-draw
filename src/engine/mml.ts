...
export function isMml(text: string): boolean {
  return /\bMML@/i.test(text);
}

export function looksLikeMml(text: string): boolean {
  return isMml(text)
    || /[<>&;,]/.test(text)
...
export function mmlPartCount(source: string): number {
  return mmlParts(source).length;
}

export function mmlPartLabels(source: string): readonly string[] {
  return mmlParts(source).map((part) => part.label);
}

function readPart(part: string): PartTiming {
  ...
  while (at < part.length) {
    ...
    if (char === '&') { tieNext = true; at++; continue; }
    if (char === '<' || char === '>') { octave += char === '>' ? 1 : -1; at++; continue; }

    if (char === 't') {
      const read = readNumber(part, at + 1);
      if (read) { tempo = read.value; at = read.at; } else at++;
      continue;
    }
    if (char === 'l') {
      const read = readNumber(part, at + 1);
    ...
    if (char === 'o') {
      const read = readNumber(part, at + 1);
      if (read) { octave = read.value; at = read.at; } else at++;
      continue;
    }
    if (char === 'v') {
      const read = readNumber(part, at + 1);
...
function readNamedNote(
  ...
  defaultDots: number,
  tempo: number,
): { readonly midi: number; readonly seconds: number; readonly at: number } | null {
  const step = STEP[part[at]!.toLowerCase()];
  if (step == null) return null;

  let next = at + 1;
  let accidental = 0;
  if (part[next] === '+' || part[next] === '#') { accidental = 1; next++; }
  else if (part[next] === '-') { accidental = -1; next++; }
...
function readDuration(
  ...
  defaultDots: number,
  tempo: number,
): { readonly seconds: number; readonly at: number } {
...
function readNumber(text: string, at: number): { readonly value: number; readonly at: number } | null {
  let next = at;
...
function readDots(text: string, at: number): { readonly count: number; readonly at: number } {
  let next = at;
