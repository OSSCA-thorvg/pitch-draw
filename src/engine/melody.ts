...
/** 한 음. `t` 는 0~1 로 정규화한 시작 시각이다. 실제 몇 초인지는 부르는 쪽이 정한다. */
...
export interface Melody {
  ...
  /**
   * 곡이 원래 몇 초짜리인가. 악보 파일에서 왔으면 tempo 로 잰 실제 길이고,
   * 표기로 넣은 것은 길이라는 게 없어 null 이다.
   *
   * **진행을 이 시간에 맡기지는 않는다.** 진행은 소리가 날 때만 나아간다 —
   * 벽시계로 끌고 가면 침묵해도 펜이 가서 "숨이 곧 시간"이 깨진다.
   * 이 값은 "얼마나 긴 곡인가"에만 쓴다.
  */
  readonly seconds: number | null;
  readonly preserveSeconds?: boolean;
}
 ...
 */
export function parseMelody(text: string, label = '직접 입력'): Melody {
  const tokens = text.trim().split(/\s+/).filter(Boolean);
  if (!tokens.length) throw new Error('멜로디가 비어 있습니다.');
  ...
  for (const token of tokens) {
    if (token === '-') {
      ...
      if (last == null) throw new Error('멜로디가 늘임표(-)로 시작할 수 없습니다.');
      beats.push(last);
      continue;
    }
...
function midiOf(token: string): number {
  const match = NOTE_PATTERN.exec(token);
  if (!match) throw new Error(`읽을 수 없는 음입니다 — "${token}"`);

  const [, name, accidental, octave] = match;
  const semitone = SEMITONE[name!.toLowerCase()];
  if (semitone == null) throw new Error(`읽을 수 없는 음입니다 — "${token}"`);
 ...
 */
export function noteAt(melody: Melody, t: number): number {
  const notes = melody.notes;
  let found = notes[0];
  if (!found) throw new Error('멜로디에 음이 하나도 없습니다.');

  const clamped = clamp01(t);
  for (const note of notes) {
    if (note.t > clamped) break;
    found = note;
  }
  return found.hz;
}
 ...
 * 여기서 끝난다.
 */
export function transposeIntoVoice(melody: Melody): Melody {
  const notes = melody.notes;
  ...
  while (
    lowest * Math.pow(2, octaves) < BOOM_LOW_HZ
    && median * Math.pow(2, octaves + 1) <= COMFORTABLE_MEDIAN_HIGH_HZ
  ) {
    octaves++;
  }
  if (octaves === 0) return melody;
 ...
 */
export function playSeconds(melody: Melody): number {
  if (melody.seconds == null) return TARGET_SECONDS;
