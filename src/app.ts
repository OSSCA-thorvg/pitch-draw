...
import { banner, overlay, showState, status, type ScreenState } from './ui/screen.js';
import type { Renderer } from './draw/renderer.js';
import { createSessionFromScore, type Session } from './engine/session.js';

export interface App {
  /** ThorVG 가 떴다. 이때부터 화면이 그려지고 그림을 받는다. */
  begin(renderer: Renderer): void;
  /** ThorVG 를 못 띄웠다. 앱은 여기서 끝난다 — 그릴 수 없으면 할 수 있는 게 없다. */
...
export function createApp(): App {
  ...
  const controls = createControls({
    ...
    loadFile: (file) => { if (!songs.fromFile(file)) loader.fromFile(file); },
  ...
  async function toggleGuide(): Promise<void> {
    if (!session || state === 'playing' || starting) return;
    if (guide.playing) {
      guide.stop();
      status('가이드 음을 멈췄습니다.');
      return;
  ...
  async function beginPlay(): Promise<void> {
    if (!session || guide.playing || starting) return;
    ...
    try {
      ...
      status(micErrorMessage(err as Error));
      console.error(err);
    } finally {
      starting = false;
      syncControls();
  ...
  function begin(next: Renderer): void {
    renderer = next;
  ...
  function failToStart(err: Error): void {
