...
export interface ScoreLoader {
  /** SVG 텍스트에서. 실패하면 사유를 띄우고 이전 그림은 그대로 둔다. */
  fromText(svgText: string, label: string): void;
  /** 고르거나 떨어뜨린 파일에서. SVG 가 아니면 열지 않는다. */
  fromFile(file: File | undefined): void;
}

export interface ScoreLoaderOptions {
  /** 지금 고른 멜로디. 그림이 들어올 때마다 새로 물어본다. */
  melody(): Melody;
  onLoaded(session: Session, label: string): void;
}

export function createScoreLoader({ melody, onLoaded }: ScoreLoaderOptions): ScoreLoader {
  function fromText(svgText: string, label: string): void {
    let session: Session;
    try {
  ...
  function fromFile(file: File | undefined): void {
    if (!file) return;
