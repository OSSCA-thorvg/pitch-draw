 ...
 */
import { svgToScore, pointOnScore } from './score.js';
import { centsBetween, offsetYFromCents } from './deviation.js';
import { noteAt, playSeconds } from './melody.js';
 ...
 */
export function createSession(svgText, melody) {
 ...
 */
export function createSessionFromScore(score, melody) {
    const first = score.points[0];
    if (!first)
        throw new Error('악보에 점이 하나도 없습니다.');
