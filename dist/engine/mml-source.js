export function mmlParts(source) {
...
function labelledParts(source) {
    const lines = source.split('\n').map((line) => line.trim()).filter(Boolean);
    if (lines[0] && /^MML@/i.test(lines[0]))
        return [];
    const parts = [];
    let label = '';
    let score = '';
...
//# sourceMappingURL=mml-source.js.map