 ...
 */
export function midiToMelody(bytes, label) {
    ...
    if (!tune.length)
        throw new Error('악보에 음이 하나도 없습니다.');
 ...
 * 선율선이 이긴다.
 */
function pickMelody(tracks) {
    let best = [];
    for (const track of tracks) {
        const voice = topVoice(track.notes);
        if (voice.length > best.length)
            best = voice;
    }
    return best;
}
/** 같은 시각에 울리는 화음은 맨 위 음만 남긴다 — 노래하는 사람이 따라가는 선이다. */
function topVoice(notes) {
    const byTick = new Map();
    for (const note of notes) {
        const current = byTick.get(note.tick);
        if (current == null || note.midi > current)
            byTick.set(note.tick, note.midi);
    }
...
function readTrack(bytes, from, to) {
    const notes = [];
    const tempos = [];
    let at = from;
    let tick = 0;
...
function readVarInt(bytes, from) {
    let value = 0;
    let at = from;
