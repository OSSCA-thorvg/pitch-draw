...
export function detectPitch(buf, sampleRate) {
    ...
    let start = 0;
    let end = size - 1;
    while (start < size / 2 && Math.abs(buf[start]) < threshold)
        start++;
    while (end > size / 2 && Math.abs(buf[end]) < threshold)
        end--;
    ...
    let lag = minLag;
    while (lag < maxLag && corr[lag] > corr[lag + 1])
        lag++;
    let bestLag = -1;
    let bestVal = 0;
...
export function hzToSolfegeName(hz) {
    const midi = midiFromHz(hz);
